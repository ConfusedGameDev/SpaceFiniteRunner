...
- **`PauseMenu`** — Esc / gamepad Start pauses active gameplay (`timeScale = 0` + `motor.Paused`, haptics reset) with Resume / Restart / Exit options — mouse-clickable and shortcut-driven (A/Esc, X/R, Y/Q). Never opens over the tuning or result screens (`CanPause` checks `motor.Paused` and `RunOver`). Spawned by `GameManager`, built from code on its own overlay canvas. `HapticsSystem` runs on unscaled time so rumble decays while paused.
