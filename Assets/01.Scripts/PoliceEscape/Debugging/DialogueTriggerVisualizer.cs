...
{
    /// <summary>
    /// Fills every live <see cref="DialogueTrigger"/> volume with a
    /// translucent orange box (plus outline) so triggers can be seen and
    /// driven through in the Game view while playing — a Gizmo can't do that.
    /// A trigger on cooldown draws dimmed, so "why didn't it fire" answers
    /// itself at a glance. Installed by the first trigger that wakes up (see
    /// <see cref="EnsureInstalled"/>), gated by
    ...
    public class DialogueTriggerVisualizer : DebugVisualizer
    {
        ...
        protected override void Rebuild()
        {
            foreach (DialogueTrigger trigger in DialogueTrigger.Active)
            {
                ...
                Color fill = trigger.IsReady ? ReadyFill : CoolingFill;
                Color edge = trigger.IsReady ? ReadyEdge : CoolingEdge;
